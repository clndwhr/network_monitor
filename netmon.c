...
void print_json(const char *iface, unsigned long rx, unsigned long tx, bool is_last_in_array) {
    time_t now = time(NULL);
    printf("{\"interface\":\"%s\",\"timestamp\":%ld,\"rx_bytes\":%lu,\"tx_bytes\":%lu}%s\n", iface, now, rx, tx, is_last_in_array ? "" : ",");
    fflush(stdout);
}
...
int get_all_ifaces(char ifaces[][32], int max_ifaces) {
    ...
    while ((dir = readdir(d)) != NULL && count < max_ifaces) {
        ...
        ifaces[count][31] = '\0';  // force null termination
        count++;
    }
    closedir(d);
    return count;
    ...
    while (1) {
        if (monitor_all) {
            ...
            for (i = 0; i < n; i++) {
                ...
                print_json(ifaces[i], rx, tx, i == n - 1);
            }
            printf("]\n");
        } else {
            unsigned long rx = 0, tx = 0;
            get_stats(iface, &rx, &tx);
            print_json(iface, rx, tx, false);
        }
        sleep(interval);
    }
    return 0;
